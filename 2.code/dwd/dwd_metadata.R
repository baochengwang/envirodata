...
station.meta =  file(URL,encoding='latin1') %>% 
  read.fwf(widths=c(6,9,8,15,12,10,42,98),header=FALSE,skip=2) %>% 
  data.table() 
...
station.meta %<>% mutate(Stationsname=trim(Stationsname),
                         Bundesland=trim(Bundesland))


# convert von_datum/bis_datum [numeric] to `Date` format
station.meta %<>% 
  mutate(von_datum=as.Date(as.character(von_datum),format='%Y%m%d'),
